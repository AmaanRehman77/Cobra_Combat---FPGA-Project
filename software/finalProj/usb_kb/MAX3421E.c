...
volatile alt_u32 *spi0_BASE  = (alt_u32*)0x60; //make a pointer to access the PIO block
...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	//write reg + 2 via SPI
	//write val via SPI
	alt_u8 writeData[2] = {reg+2,val};
	int ret_code = alt_avalon_spi_command(spi0_BASE, 0, 2, writeData, 0, NULL, 0);

	//read return code from SPI peripheral (see Intel documentation) 
	//if return code < 0 print an error
	if (ret_code < 0) {
		printf("Error writing register to MAX3421E");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	alt_u8 writeData[nbytes+1];
	writeData[0] = reg+2;
	for(int i = 0; i < nbytes; i++){
		writeData[i+1] = data[i];
	}
	int ret_code = alt_avalon_spi_command(spi0_BASE, 0, nbytes+1, writeData, 0, NULL, 0);


	//read return code from SPI peripheral (see Intel documentation) 
	//if return code < 0  print an error
	if (ret_code < 0) {
		printf("Error writing bytes(mult) to MAX3421E");
	}
	//deselect MAX3421E (may not be necessary if you are using SPI peripheral)
		// NOT DONE
...
BYTE MAXreg_rd(BYTE reg) {
	...
	//read val via SPI
	alt_u8 val;
	int ret_code = alt_avalon_spi_command(spi0_BASE, 0, 1, &reg, 1, &val, 0);

	//read return code from SPI peripheral (see Intel documentation)
	//if return code < 0 print an error
	if (ret_code < 0) {
		printf("Error reading register from MAX3421E");
	}
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	int ret_code = alt_avalon_spi_command(spi0_BASE, 0, 1, &reg, nbytes, data, 0);

	//read return code from SPI peripheral (see Intel documentation)
	//if return code < 0 print an error
	if (ret_code < 0) {
		printf("Error reading BYTES(MULT) from MAX3421E");
	}
